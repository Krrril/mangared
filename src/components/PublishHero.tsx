...
// 6 реально опубликованных тайтлов (см. задачу про замену стоковых панелей,
// и про добавление шестого — LATENT OF HEMA — первым в списке) — обложки
// скачаны из R2 и пережаты локально в WebP (см. предыдущие задачи,
// scripts/optimize-hero-covers.mjs каждый раз удалялся после использования),
// не отдаются напрямую с R2: один из оригиналов весил 3.2MB, у LATENT OF
// HEMA — 1.7MB. author — только для мобильного слайдера (см. ниже,
// "Название тайтла и автор"), десктопная раскладка панелей его не показывает.
const FEATURED_TITLES: FeaturedTitle[] = [
  {
    id: '2eef94a9-2277-4b6a-ab7b-1cd94e298ad6',
    title: 'LATENT OF HEMA',
    author: 'Farlo Alvaro',
    cover: '/hero-authors/latent-of-hema.webp',
  },
 ...
 * PublishHero.module.css, именно по ширине экрана, а не по hover-
 * возможности. Более новая задача (добавление 6-й панели) снова просила
 * гейтить по (hover: hover) and (pointer: fine) — сознательно ОСТАВЛЕНО
 * по ширине: это тот же спорный пункт, что уже разбирался и был намеренно
 * изменён в задаче про мобильный слайдер (тачскрин-ноутбук с мышью иначе
 * увидел бы раскладку из панелей вместо слайдера), а новая формулировка
 * похожа на повтор старого ТЗ, а не на осознанный откат того решения.
 * Hover-возможность по-прежнему участвует ниже — но только в раздвижении
 * панели при наведении (см. .panel:hover), а не в переключении раскладок
 * целиком.
 * - .panels — раскладка из 6 панелей с раздвижением при наведении
 *   (десктоп, ≥768px).
 * - .slider — автопрокручивающийся слайдер на один тайтл (мобильный,
 *   <768px), см. ниже.
